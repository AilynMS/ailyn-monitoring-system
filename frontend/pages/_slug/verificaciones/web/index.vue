<template>
  <div class="home-wrapper">
    ...
    <Breadcrumb 
      v-if="!isEmpty" 
      :urls="[{ path: 'web', name: 'Verificaciones Web', last: true }]"/>
    ...
    <!-- if isEmpty -->
    <EmptyScreen title="" subtitle="" type="Web" v-if="isEmpty">
          <Button
            @click="toggleModal" 
            type="primary-light" 
    ...
    <AmsTable v-if="!isEmpty" :header="[
      ...
      <AmsRow v-for="(val, index) in searchFilter" :key="index">
        ...
        <AmsColumn>{{ val.ipv6 ? 'IPv6' : 'IPv4' }}</AmsColumn>
        <AmsColumn>{{ val.https ? 'HTTPS' : 'HTTP' }}</AmsColumn>
        ...
        <AmsColumn :hoverable="true">
          <nuxt-link 
            ...
            title="Estadísticas" 
            :to="`/${$auth.user && $auth.user.tenant.slug}/verificaciones/web/${val.id}/estadisticas`"> 
            <fa icon="chart-bar" /> 
          </nuxt-link>
      ...
      <ValidationObserver ref="form">
        <form action autocomplete="nope">
            ...
            <div class="form-items mt-5">
                <label class="block text font-medium leading-5" for="urlOrIp">
                    URL o IP
                </label>

                <ValidationProvider rules="required|url_validate|domain_validate" v-slot="{errors}">
                    <div class="mt-1 rounded-md shadow-sm">
                        <input 
            ...
            <ModalCollapsable title="Más opciones">
              ...
              <div class="form-items mt-5">
                <label class="block text font-medium leading-5" for="protocol">
              ...
              <div class="form-items mt-5">
                  ...
                  <div class="mt-1 rounded-md">
                      <ValidationProvider v-slot="{errors}">
                          <v-select
                              class="m:text-sm sm:leading-5 rounded-md"
                              :options="[{code: 0, name: 'HTTP'}, { code: 1, name: 'HTTPS'}]"
              ...
              <div class="form-items mt-5">
                  <label class="block text font-medium leading-5" for="path">
                      Path
                  </label>

                  <ValidationProvider rules="path_validate" v-slot="{errors}">
                      <div class="mt-1 rounded-md shadow-sm">
                          <input 
            ...
            <div class="form-items mt-5">
                <label class="block text font-medium leading-5" for="requestHeaders">
                  Cabeceras de solicitud (opcional)
                </label>
            ...
            <div class="form-items mt-5">
                <label class="block text font-medium leading-5" for="tags">
                  Tags (opcional)
                </label>
...
export default {
  ...
  head() {
    return {
      title: `Verificaciones Web | AilynMS`
    }
  },
  ...
  computed: {
    ...
    getModalTitle(){
      if(this.editMode) return 'Modificar Verificación Web';
      else return 'Añadir Verificación Web';
    }
  },
  ...
  methods: {
    ...
    async handleStatus(item) {
      ...
      let url =`${this.$auth.user.tenant.slug}/verifications/web-verifications/${this.form.id}`;
    ...
    async handleSubmit() {
      ...
      if(isCompleted) {
        let url =`${this.$auth.user.tenant.slug}/verifications/web-verifications`;
    ...
    handleDelete(id) {
      this.isLoading = true;
      const url = `${this.getTenant.slug}/verifications/web-verifications/${id}`;
    ...
    getRecords() {
      const url = `${this.getTenant.slug}/verifications/web-verifications?page=${this.meta.current_page}`;
