<template>
    <div class="ping-wrapper">
        <Breadcrumb 
            v-if="!isEmpty" 
            :urls="[{ path: 'ping', name: 'Verificaciones Ping', last: true }]"/>
        ...
        <!-- if isEmpty -->
        <EmptyScreen title="" subtitle="" type="Ping" v-if="isEmpty">
            <Button
                @click="toggleModal" 
                type="primary-light" 
            ...
            <AmsRow v-for="(val, index) in searchFilter" :key="index">
                ...
                <AmsColumn>
                    <nuxt-link 
                        ...
                        title="Estadísticas" 
                        :to="`/${$auth.user && $auth.user.tenant.slug}/verificaciones/ping/${val.id}/estadisticas`"> 
                        <fa icon="chart-bar" /> 
                    </nuxt-link>
...
export default {
    ...
    head() {
        return {
            title: `Verificaciones Ping | AilynMS`
        }
    },
    ...
    computed: {
        ...
        getModalTitle() {
            if(this.editMode) return 'Modificar Verificación Ping';
            else return 'Añadir Verificación Ping';
        }
    },
    ...
    methods: {
        ...
        async handleStatus(item) {
            ...
            let url =`${this.$auth.user.tenant.slug}/verifications/icmp-verifications/${this.form.id}`;
        ...
        async handleSubmit() {
            ...
            if(isCompleted) {
                let url =`${this.$auth.user.tenant.slug}/verifications/icmp-verifications`;
        ...
        handleDelete(id) {
            this.isLoading = true;
            const url = `${this.getTenant.slug}/verifications/icmp-verifications/${id}`;
        ...
        getRecords() {
            const url = `${this.getTenant.slug}/verifications/icmp-verifications?page=${this.meta.current_page}`;
