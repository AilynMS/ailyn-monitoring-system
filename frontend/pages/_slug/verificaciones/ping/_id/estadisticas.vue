<template>
    <div class="stats-wrapper" v-if="records">
        <Breadcrumb 
            :urls="[
                { path: 'ping', name: 'Verificaciones Ping'}, 
                { path: `ping/${records.id}/estadisticas`, name: `${records.name ? records.name : 'Estadísticas'}`, last: true}]"/>
        ...
        <div v-if="chartInfoOK" class="chartWrapper bg-white rounded-lg shadow-lg py-10 mt-5 relative">
            <PingLineChart 
                ...
                :userInfo="records && records.ping_verification"/>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        getRecords() {
            this.$axios.$get(`${this.getTenant.slug}/verifications/icmp-verifications/${this.idParam}`)
                .then(res => this.records = res.data.icmp_verification)
                .catch(error => error);
        },
        ...
        getData() {
            ...
            return this.$axios.$get(`${this.getTenant.slug}/verifications/icmp-verifications/${this.idParam}/get-data?start_date=${this.dateFilteredStart || 0}&end_date=${this.dateFilteredEnd || 0}`)
                .then(res => {
                    this.chartInfoOK = res.data.result.filter(res => {
                        return res.is_alive === true;
                    });

                    this.chartInfoFail = res.data.result.filter(res => {
                        return res.is_alive === false;
                    });
                })
                .catch(error => error);
        },
