<template>
    <div class="stats-wrapper" v-if="records">
        <Breadcrumb 
            :urls="[
                { path: 'TCP', name: 'Verificaciones TCP/UDP'}, 
                { path: `TCP/${records.id}/estadisticas`, name: `${records.name ? records.name : 'Estadísticas'}`, last: true}]"/>
        ...
        <div v-if="chartInfoOK" class="chartWrapper bg-white rounded-lg shadow-lg py-10 mt-5 relative">
            <TCPLineChart 
                ...
                :userInfo="records && records.TCP_verification"/>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        getRecords() {
            this.$axios.$get(`${this.getTenant.slug}/verifications/tcp-verifications/${this.idParam}`)
                .then(res => this.records = res.data.TCP_verification)
                .catch(error => error);
        },
        ...
        getData() {
            ...
            return this.$axios.$get(`${this.getTenant.slug}/verifications/tcp-verifications/${this.idParam}/get-data?start_date=${this.dateFilteredStart || 0}&end_date=${this.dateFilteredEnd || 0}`)
                .then(res => {
