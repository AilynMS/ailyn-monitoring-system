<template>
  <div class="home-wrapper">
    ...
    <Breadcrumb 
      v-if="!isEmpty" 
      :urls="[{ path: 'tcp', name: 'Verificaciones TCP/UDP', last: true }]"/>
    ...
    <!-- if isEmpty -->
    <EmptyScreen title="" subtitle="" type="TCP/UDP" v-if="isEmpty">
          <Button
            @click="toggleModal" 
            type="primary-light" 
    ...
    <AmsTable v-if="!isEmpty" :header="[
      ...
      <AmsRow v-for="(val, index) in searchFilter" :key="index">
        ...
        <AmsColumn>{{ val.type ? 'TCP' : 'UDP' }}</AmsColumn>
        ...
        <AmsColumn :hoverable="true">
          <nuxt-link 
            ...
            title="Estadísticas" 
            :to="`/${$auth.user && $auth.user.tenant.slug}/verificaciones/TCP/${val.id}/estadisticas`"> 
            <fa icon="chart-bar" /> 
          </nuxt-link>
        ...
        <form action autocomplete="nope">
            ...
            <div class="form-items mt-5">
                ...
                <ValidationProvider rules="required" v-slot="{errors}">
                    <div class="mt-1 rounded-md shadow-sm">
                        <input 
                            placeholder="http://www.misitio.com"
                            v-model="form.target"
                            v-bind:class="errors[0] ? 'border-red-500 focus:border-red-500 border' : 'border focus:border-gray-500'"
                            class="block w-full px-3 py-2 border-gray-300 rounded-md placeholder-gray-500 focus:outline-none transition duration-150 ease-in-out sm:text-sm sm:leading-5"
                            id="urlOrIp" 
              ...
              <div class="form-items mt-5">
                  ...
                  <div class="mt-1 rounded-md">
                      <ValidationProvider v-slot="{errors}">
                          <v-select
                              class="m:text-sm sm:leading-5 rounded-md"
                              :options="[{code: 0, name: 'TCP'}, { code: 1, name: 'UDP'}]"
...
export default {
  ...
  head() {
    return {
      title: `Verificaciones TCP/UDP | AilynMS`
    }
  },
  ...
  computed: {
    ...
    getModalTitle(){
      if(this.editMode) return 'Modificar Verificación TCP/UDP';
      else return 'Añadir Verificación TCP/UDP';
    }
  },
  ...
  methods: {
    ...
    async handleStatus(item) {
      ...
      let url =`${this.$auth.user.tenant.slug}/verifications/tcp-verifications/${this.form.id}`;
    ...
    async handleSubmit() {
      ...
      if(isCompleted) {
        let url =`${this.$auth.user.tenant.slug}/verifications/tcp-verifications`;
    ...
    handleDelete(id) {
      this.isLoading = true;
      const url = `${this.getTenant.slug}/verifications/tcp-verifications/${id}`;
    ...
    getRecords() {
      const url = `${this.getTenant.slug}/verifications/tcp-verifications?page=${this.meta.current_page}`;
