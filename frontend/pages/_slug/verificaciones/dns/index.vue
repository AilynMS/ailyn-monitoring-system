<template>
    <div class="ping-wrapper">
        <Breadcrumb 
            v-if="!isEmpty" 
            :urls="[{ path: 'dns', name: 'Verificaciones DNS', last: true }]"/>
        ...
        <!-- if isEmpty -->
        <EmptyScreen title="" subtitle="" type="DNS" v-if="isEmpty">
            <Button
                @click="toggleModal" 
                type="primary-light" 
        ...
        <AmsTable v-if="!isEmpty" :header="[
            ...
            <AmsRow v-for="(val, index) in searchFilter" :key="index">
                ...
                <AmsColumn>{{ val.expected_response }}</AmsColumn>
                ...
                <AmsColumn>
                    <nuxt-link 
                        ...
                        title="Estadísticas" 
                        :to="`/${$auth.user && $auth.user.tenant.slug}/verificaciones/dns/${val.id}/estadisticas`"> 
                        <fa icon="chart-bar" /> 
                    </nuxt-link>
            ...
            <ValidationObserver ref="form">
                <form action autocomplete="nope">
                    <div class="form-items">
                        ...
                        <ValidationProvider rules="required" v-slot="{errors}">
                            <div class="mt-1 rounded-md shadow-sm">
                                <input 
                                    placeholder="¡Mi primera verificación!"
                                    v-model="form.name"
                    ...
                    <div class="form-items mt-5">
                        <label class="block text font-medium leading-5" for="hostIp">
                            Host/IP
                        </label>
                    ...
                    <div class="form-items mt-5">
                        <label class="block text font-medium leading-5" for="response">
                            Respuesta esperada
                        </label>
                        ...
                        <ValidationProvider rules="required" v-slot="{errors}">
                            <div class="mt-1 rounded-md shadow-sm">
                                <input 
                                    ...
                                    v-model="form.expected_response"
                                    v-bind:class="errors[0] ? 'border-red-500 focus:border-red-500 border' : 'border focus:border-gray-500'"
                                    class="block w-full px-3 py-2 border-gray-300 rounded-md placeholder-gray-500 focus:outline-none transition duration-150 ease-in-out sm:text-sm sm:leading-5"
                                    id="response" 
                                    type="text">
...
export default {
    ...
    head() {
        return {
            title: `Verificaciones DNS | AilynMS`
        }
    },
    ...
    computed: {
        ...
        getModalTitle() {
            if(this.editMode) return 'Modificar Verificación DNS';
            else return 'Añadir Verificación DNS';
        }
    },
    ...
    methods: {
        ...
        async handleStatus(item) {
            ...
            let url =`${this.$auth.user.tenant.slug}/verifications/dns-verifications/${this.form.id}`;
        ...
        async handleSubmit() {
            ...
            if(isCompleted) {
                let url =`${this.$auth.user.tenant.slug}/verifications/dns-verifications`;
                ...
                try {
                    ...
                    if(err.response) return this.handleError(err.response.data); 
        ...
        handleDelete(id) {
            this.isLoading = true;
            const url = `${this.getTenant.slug}/verifications/dns-verifications/${id}`;
        ...
        getRecords() {
            const url = `${this.getTenant.slug}/verifications/dns-verifications?page=${this.meta.current_page}`;
